...
blocklist_file_name = "root/input_list_roots.txt"
...
sorted_list = "root/sorted_list_roots.txt"
sorted_list_w_hash = "root/sorted_hash_list_roots.txt"
...
def gen_merkle_tree(file_name):
    ...
    f = open("root/merkle_tree_roots.txt","w")
    f.write(str(tree_struct))
    f.close()

def get_dir_and_auth_path(leaf_idx):
    with open("root/merkle_tree_roots.txt", 'rb') as readFile:
        tree_struct = eval(readFile.read())
...
# 	leaf = 10742206328459494425512705963144036433234462290484594732452839713647761096654
...
if __name__ == "__main__":
    ...
    values=[direction,auth_path,root]
    
    f = open("../zkp/prover_inputs/token_revocation.txt","a")
    for line in values:
        f.write(str(line)+'\n')
    f.close()

    ## verify_path for leaf
    # print(verify_path([leaf], auth_path, direction, height, root))x
