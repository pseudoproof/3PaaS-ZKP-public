...
use rug::Integer as rug_int;

// use poseidon::pack_vector;
...
// change to poseidon hash
pub fn mgf1_xor_poseidon(out: &mut [u8], digest: &mut dyn DynDigest, seed: &[u8]) {
    ...
    while i < out.len() {
        ...
        // println!("{:?}", digest_input);
        let digest_input_unpacked: Vec<rug_int> = digest_input.iter().map(|&b| rug_int::from(b)).collect();
        let digest_input_packed = poseidon::pack_vector(digest_input_unpacked.clone(), 248, 8);
        
        // digest.update(digest_input.as_slice());
        // let digest_output = &*digest.finalize_reset();
       
        let digest_output:Vec<u8> = poseidon::poseidon_hash(digest_input_packed).to_digits(rug::integer::Order::MsfBe);
...
fn emsa_pss_encode(
    ...
    // poseidon is also 32 byte (255 bits but we will pad the last bit with 0)
    ...
    let int_prefix_unpacked: Vec<rug_int> = prefix.iter().map(|&b| rug_int::from(b)).collect();
    let int_prefix_packed = poseidon::pack_vector(int_prefix_unpacked, 248, 8);

    let int_salt_unpacked: Vec<rug_int> = salt.iter().map(|&b| rug_int::from(b)).collect();
    let int_salt_packed = poseidon::pack_vector(int_salt_unpacked, 248, 8);
    
    let tmp = vec![int_prefix_packed[0].clone(), m_hash.clone(), int_salt_packed[0].clone()];
    let hashed = poseidon::poseidon_hash(tmp).to_digits(rug::integer::Order::MsfBe);
...
impl PublicKey {
    ...
    pub fn to_limb(input: Vec<u8>) -> Vec<u128>{
        // Two function below from chatgpt:
        let mut bit_values: Vec<u8> = input.iter()
        .flat_map(|&byte| {
            (0..8).map(move |i| {
                (byte >> (7 - i)) & 1
            })
        ...
        for _i in 0..60 {
            bit_values.insert(0,0);
        }
        
        let mut decimal_values: Vec<u128> = Vec::new();

        for chunk in bit_values.chunks(124) {
            let mut value = 0u128;
            for (i, &bit) in chunk.iter().enumerate() {
                value |= (bit as u128) << (123 - i); 
            }
            decimal_values.push(value);
        }
        decimal_values.reverse();
        decimal_values
    }
    ...
    pub fn blind<R: CryptoRng + RngCore>(
        ...
        let seed: u64 = 41; //constant randomizer 
        let mut msg_rng = StdRng::seed_from_u64(seed);
        let mut noise = [0u8; 32];
        let msg_randomizer = if randomize_message {
            msg_rng.fill(&mut noise[..]);
            Some(MessageRandomizer(noise))
        } else {
            None
        };
        println!("msg_prefix {:?}", msg_randomizer.unwrap().0);
        lines.push(format!("{:?}", msg_randomizer.unwrap().0));

        let tmp_msg:[u8;8] = msg.try_into().expect("error");
        let unpacked_msg: Vec<u8> = noise.iter().chain(tmp_msg.iter()).copied().collect();
        let int_unpacked_msg: Vec<rug_int> = unpacked_msg.iter().map(|&b| rug_int::from(b)).collect();
        let int_packed_msg = poseidon::pack_vector(int_unpacked_msg, 248, 8);
        let msg_hash = poseidon::poseidon_hash(int_packed_msg); 
        
        let seed: u64 = 40; //constant randomizer 
        let mut salt_rng = StdRng::seed_from_u64(seed);
        let salt_len = 15;
        let mut salt = vec![0u8; salt_len];
        salt_rng.fill(&mut salt[..]);
        println!("salt {:?}", salt.clone());
        lines.push(format!("{:?}", salt.clone()));
        let padded = emsa_pss_encode(&msg_hash, modulus_bits - 1, &salt, &mut Sha256::new());

        let m = BigUint::from_bytes_be(&padded);
        // println!("m {}", m);
        let decimal_values = Self::to_limb(padded.clone());
        // println!("EM (padded and reversed ++ for zok): {:?}", decimal_values);
        lines.push(format!("{:?}", decimal_values));
        // println!("N :{}", self.0.n());

        let n_limbs = Self::to_limb(self.0.n().to_bytes_be_padded(modulus_bytes));
        // println!("N limbs :{:?}", n_limbs);

        if m.gcd(self.0.n()) != BigUint::one() {
            return Err(Error::UnsupportedParameters);
        }

        // ------- this code is added from rsa -- internal.rs to print about the r^e
        let mut r: BigUint;
        let mut ir: Option<BigInt>;
        let secret;
        loop {
            r = rng.gen_biguint_below(self.as_ref().n());
            if r.is_zero() {
                r = BigUint::one();
            }
            ir = r.clone().mod_inverse(self.as_ref().n());
            if let Some(ir) = ir {
        ...
        let blind_msg = {
            let r_limbs = Self::to_limb(r.clone().to_bytes_be_padded(modulus_bytes));
            // println!("r {:?}", r_limbs);
            lines.push(format!("{:?}", r_limbs));
            let mut rpowe = r.modpow(self.as_ref().e(), self.as_ref().n()); // N != 0
            let tmp = Self::to_limb(rpowe.to_bytes_be_padded(modulus_bytes)); 
            // println!("blinder: {:?}", tmp);
            lines.push(format!("{:?}", tmp));
            let mut c = m.clone() * &rpowe;
            c %= self.as_ref().n();
            //for testing the product uncomment the two lines below:
            let tmp2 = Self::to_limb(c.to_bytes_be_padded(modulus_bytes)); 
            // println!("blinded: {:?}", tmp2);
            lines.push(format!("{:?}", tmp2));
            rpowe.zeroize();
            c
        };
    ...
    pub fn verify(
        ...
        let verified = match Hash::Sha256 {
            Hash::Sha256 => {
