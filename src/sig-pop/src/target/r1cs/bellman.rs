...
use super::proof;
use super::{wit_comp::StagedWitCompEvaluator, Lc, ProverData, ProverDataBellman, Var, VarType, VerifierData};
...
pub(super) fn int_to_ff<F: PrimeField>(i: Integer) -> F {
    ...
    let result = F::from_repr_vartime(repr).unwrap();
    result
}
...
pub struct SynthInput<'a, F: PrimeField>(&'a ProverDataBellman<F>, Option<&'a FxHashMap<String, Value>>);

impl<'a, F: PrimeField> Circuit<F> for SynthInput<'a, F> {
    #[track_caller]
        ...
        CS: ConstraintSystem<F>,
    {
        ...
        let twv = start_timer!(|| "wit gen & var creation");
        let t = start_timer!(|| "wit gen");
        ...
        end_timer!(t);
        let t = start_timer!(|| "var creation");
        ...
        end_timer!(t);
        end_timer!(twv);
        let t = start_timer!(|| "lc enforcement");
        for (i, (a, b, c)) in self
            .0
            .r1cs
            .constraints
...
// pub struct Bellman<E: Engine, F: PrimeField>(PhantomData<E>, PhantomData<F>);
pub struct Bellman<E: Engine>(PhantomData<E>);

/// The pk for [Bellman]
#[derive(Serialize, Deserialize)]
pub struct ProvingKey<E: Engine, F: PrimeField>(
    ProverDataBellman<F>,
...
where
    ...
    E::G1: WnafGroup,
    E::G2: WnafGroup,
    E::Fr: PrimeFieldBits + Serialize + for<'a> Deserialize<'a>,
{
    type VerifyingKey = VerifyingKey<E>;

    type ProvingKey = ProvingKey<E, E::Fr>;
    ...
    fn setup(p_data: ProverData, v_data: VerifierData) -> (Self::ProvingKey, Self::VerifyingKey) {
        ...
        let p_data_setup = ProverDataBellman::from_proverdata::<groth16::generator::KeypairAssembly<E::Fr>>(&p_data);
        let params =
            groth16::generate_random_parameters::<E, _, _>(SynthInput(&p_data_setup, None), rng).unwrap();
        let v_params = params.vk.clone();
        let p_data_prove = ProverDataBellman::from_proverdata::<groth16::prover::ProvingAssignment<E::Fr>>(&p_data);
        (ProvingKey(p_data_prove, params), VerifyingKey(v_data, v_params))
    }

    fn prove(pk: &Self::ProvingKey, witness: &FxHashMap<String, Value>) -> Self::Proof {
        let rng = &mut rand::thread_rng();
        let t = crate::start_timer!(|| "check");
        // pk.0.check_all(witness);
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "proof");
        let p = Proof(groth16::create_random_proof(SynthInput(&pk.0, Some(witness)), &pk.1, rng).unwrap());
        crate::end_timer!(t);
        p
    }

    fn verify(vk: &Self::VerifyingKey, inst: &FxHashMap<String, Value>, pf: &Self::Proof) -> bool {
        let t = crate::start_timer!(|| "prepare_verifying_key");
        let pvk = groth16::prepare_verifying_key(&vk.1);
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "eval inst");
        let r1cs_inst_map = vk.0.eval(inst);
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "map r1cs_inst");
        let r1cs_inst: Vec<E::Fr> = r1cs_inst_map
            .into_iter()
            .map(|i| int_to_ff(i.i()))
            .collect();
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "verify proof");
        let result = groth16::verify_proof(&pvk, &pf.0, &r1cs_inst).is_ok();
        crate::end_timer!(t);
        result
    }
}
