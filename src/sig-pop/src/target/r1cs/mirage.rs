...
use ::bellman::{
    ...
    kw15, mirage, LinearCombination, SynthesisError,
};
...
use super::proof;
use super::{
    wit_comp::StagedWitCompEvaluator, ProverData, ProverDataBellman, VarType, VerifierData,
};
...
/// bellman prover.
pub struct SynthInput<'a, F: PrimeField>(
    /// The prover data
    // &'a ProverData,
    &'a ProverDataBellman<F>,
    /// The inputs
...
impl<'a, F: PrimeField + PrimeFieldBits> CcCircuit<F> for SynthInput<'a, F> {
    ...
    {
        let t_synth = crate::start_timer!(|| "synth");
        ...
        let t = start_timer!(|| "wit gen & var creation");
        let mut var_idx = 0;
                        ...
                        *num_cwits.last_mut().unwrap() -= 1;
                        ...
                        while let Some(n) = num_cwits.last() {
                            if n == &0 {
                                ...
                                let t = crate::start_timer!(|| "end_block");
...
#[serde(bound = "")]
pub struct ProvingKey<E: Engine, F: PrimeField + Serialize + for<'a> Deserialize<'a>> {
    data: ProverDataBellman<F>,
    #[serde(with = "serde_pk")]
    ...
    E::Fr: PrimeFieldBits + Serialize + for<'a> Deserialize<'a>,
{
    ...
    fn cp_setup(
        ...
        let data_len = p_data
            ...
            .map(|c| c.len())
            .unwrap_or(0);
        let p_data_setup = ProverDataBellman::from_proverdata::<
            mirage::generator::KeypairAssembly<E::Fr>,
        >(&p_data);
        let params = mirage::generate_random_parameters::<E, _, _>(
            SynthInput(&p_data_setup, None, None),
            rng,
        )
        .unwrap();
        let cks = (0..num_cmts)
        ...
        let p_data_prove = ProverDataBellman::from_proverdata::<
            mirage::prover::ProvingAssignment<E, &mirage::Parameters<E>>,
        >(&p_data);
        (
            ProvingKey {
                data: p_data_prove,
    ...
    fn cp_prove(
        ...
        // assert_eq!(rand.len(), pk.data.num_commitments()); // remove this for now
        let rng = &mut rand::thread_rng();
        let t_ck = crate::start_timer!(|| "check_all");
        // #[cfg(debug_assertions)]
        // pk.data.check_all(witness);
        crate::end_timer!(t_ck);
        let rands: Vec<E::Fr> = rand.iter().map(|r| r.0).collect();
        let mut rng = &mut rand::thread_rng();
        let pf_rands: Vec<E::Fr> = (0..rand.len()).map(|_| E::Fr::random(&mut *rng)).collect();
        let t_pf = crate::start_timer!(|| "proof");
        ...
        let t_cpl = crate::start_timer!(|| "cp_link");
        let link = cp_link::prove(&pk.link, rands, pf_rands, aux_blocks);
        crate::end_timer!(t_cpl);
        Proof {
            mirage: mirage_pf,
            link,
        }
    ...
    fn cp_verify(
        ...
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "inst eval");
        let r1cs_inst_map = vk.data.eval(inst);
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "map inst");
        let r1cs_inst: Vec<E::Fr> = r1cs_inst_map
            .into_iter()
            .map(|i| int_to_ff(i.i()))
            .collect();
        crate::end_timer!(t);
        let t = crate::start_timer!(|| "verify");
        let result = mirage::verify_proof(&pvk, &pf.mirage, &r1cs_inst).is_ok();
        crate::end_timer!(t);
        result
    }
