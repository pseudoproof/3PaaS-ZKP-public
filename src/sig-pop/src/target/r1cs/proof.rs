...
pub trait ProofSystem {
    ...
    fn prove_fs(
        ...
        let s = crate::start_timer!(|| "read_pk");
        let pk: Self::ProvingKey = deserialize_from_file(pk_path)?;
        crate::end_timer!(s);
        let s = crate::start_timer!(|| "read_wit");
        let witness = value_map_from_path(witness_path)?;
        crate::end_timer!(s);
        let s = crate::start_timer!(|| "prove");
        let start = Instant::now();
        let pf = Self::prove(&pk, &witness);
        let duration = start.elapsed();
        println!("Time taken: {:?}", duration);
        crate::end_timer!(s);
        let s = crate::start_timer!(|| "write_pf");
        let result = serialize_into_file(&pf, pf_path);
        crate::end_timer!(s);
        result
    }
    /// Verify from files
    fn verify_fs(
        ...
        let start = Instant::now();
        let result = Ok(Self::verify(&vk, &instance, &pf));
        let duration = start.elapsed();
        println!("Time taken: {:?}", duration);
        result
    }
    /// Prove to/from files except that witness is computed in the code
    fn prove_fs_outofcir(
        pk_path: impl AsRef<Path>,
        ...
        let pk: Self::ProvingKey = deserialize_from_file(pk_path)?;
        let start = Instant::now();
        let pf = Self::prove(&pk, &input_map);
        let duration = start.elapsed();
        let result = serialize_into_file(&pf, pf_path);
        // let duration = start.elapsed();
        println!("Time taken for prove_fs_outofcir: {:?}", duration);    
        result      
    }
    ///  Verify from files except that the input is computed in the code
    fn verify_fs_outofcir(
        vk_path: impl AsRef<Path>,
        input_map: HashMap<String, Value>,
        pf_path: impl AsRef<Path>,
    ) -> std::io::Result<bool> {
        ...
        let pf: Self::Proof = deserialize_from_file(pf_path)?;
        let start = Instant::now();
        let result = Self::verify(&vk, &input_map, &pf);
        let duration = start.elapsed();
        println!("Time taken for verify_fs_outofcir: {:?}", duration);            
        Ok(result)
    }
}
...
pub trait CommitProofSystem {
    ...
    fn cp_prove_fs(
        ...
        let s = crate::start_timer!(|| "cp_prove");
        let pf = Self::cp_prove(&pk, &witness, &rands);
        crate::end_timer!(s);
        let s = crate::start_timer!(|| "write_pf");
        let r = serialize_into_file(&pf, pf_path);
        crate::end_timer!(s);
        r
    }
    /// Verify from files
